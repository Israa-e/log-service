...
**Tried and reverted: COPY + request coalescing on the write path.** Postgres CPU sits at 66-95% under load, which was suspected to be starving `GET /logs/aggregate` of CPU time (both share the single core). Replaced `INSERT ... SELECT * FROM unnest(...)` with `COPY` (bypasses the SQL parser/planner) plus coalescing concurrent requests' rows into one `COPY` per 10ms flush, to amortize per-statement cost across more rows than one ~33-row HTTP request.

- **Attempt 1** passed a 40-worker/10,000-row concurrency test cleanly (100% accepted, zero missing, zero duplicates), but a run through `logs-benchmark-cli`'s full scenario suite showed aggregate p95 spiking to a 60s client timeout with new errors appearing.
- **Attempt 2** added the safeguards that should have prevented that: a hard timeout with forced connection destruction on the flush itself (verified directly — an artificially-broken flush fails every request within ~100ms instead of hanging, and the server stays fully responsive), a cap on rows per flush, and a bounded queue that sheds new requests once full. It passed the same concurrency test, plus 90 seconds of sustained concurrent load locally with zero failures and an exact rollup/raw-count match.

The confounder that surfaced afterward: the dev machine this was built and tested on was running a fully loaded browser, editor, and chat client throughout (61 Chrome/Discord/VS Code processes, at times under 900MB free RAM with 1.7GB swapped), and re-running the *unmodified, already-shipped* `INSERT`-only baseline reproduced the same extreme-latency spikes on the same tool. Three full runs of attempt 2 then landed 61.1, 61.0, and 60.4-with-a-60s-aggregate-spike back to back on the same code — two clean, one not, which fits host contention better than a deterministic bug in the batching itself. It's unresolved which one it actually was, and that uncertainty is the point: instability that shows up intermittently on a graded submission isn't worth shipping regardless of its cause. Reverted for the last time; `INSERT ... unnest` is what's running. If revisited, it should be measured on a machine that isn't also running a browser — the course's own guidance says background load alone swung a score by 11 points on the staff's test machine.

**Named prepared statements for the two constant-shape hot-path queries** (the raw insert and the rollup flush upsert) — `pg` parses and plans a named statement once per physical connection instead of once per call, cutting Postgres CPU per query without changing timing, concurrency, or the response path at all, unlike the reverted COPY attempt above. Verified under 90s of sustained concurrent load (30 workers, ~9,500 requests, 312,048 rows) with zero failures and an exact rollup/raw-count match, and under a 10,000-row/40-worker concurrency burst with zero missing and zero duplicates.
